...
loadingManager.onProgress=(url, loaded, total)=>{
  ...
  loadTxt.textContent='加载�?'+pct+'%';
};

const loader=new GLTFLoader(loadingManager);

// ── 状态机 ──
// 每组4个阶�? rise �?explode �?implode �?fall
const PHASES=['上升', '散开', '收回', '下落'];
let groups=[];        // 3个Group对象
let groupData=[];     // 每个Group的动画数�?
...
async function init(){
  ...
  // �?name 排序确保 Group1 < Group2 < Group3
  ...
  groupData=groups.map(group=>{
    ...
    return {
      ...
      size:gSize.clone(),
      riseDist:gSize.y * 1.5,           // 上升距离（组高度�?.5倍）
  ...
  if(phase===0){
    ...
    runAnim(700, t=>{
      ...
      if(currentGroup>=groups.length){
        ...
        updateTip('点击屏幕：第'+(currentGroup+1)+'组上�?);
...
function handleClick(){
  ...
  if(currentPhase<4){
    ...
    currentPhase++;
    if(currentPhase>3) currentPhase=0; // 会在 doPhase �?onComplete 里更�?currentGroup
  }
}

function updateTip(text){
  tip.textContent=text;
  titleSub.textContent='�?+(currentGroup+1)+'�?/ �?�?;
}
